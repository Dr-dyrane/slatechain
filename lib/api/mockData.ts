...
import {
  ...
  const randomPhone = (): string => {
	  return `+1${Math.floor(Math.random() * 1000)}-${Math.floor(Math.random() * 1000)}-${Math.floor(Math.random() * 10000)}`.padEnd(14, '0');
  };
  ...
  const generateWarehouses = (count: number): Warehouse[] => {
	  ...
	  for (let i = 0; i < count; i++) {
		  ...
		  for (let j = 0; j < zoneCount; j++) {
			  const zoneTypes = ["BULK", "PICKING", "COLD_STORAGE", "HAZMAT", "HIGH_VALUE"];
  ...
  const generateSuppliers = (count: number): Supplier[] => {
	  ...
	  for (let i = 0; i < count; i++) {
		  ...
		  const contactPerson = customerNames[randomNumber(0, customerNames.length - 1)];
  ...
  const generateShipments = (count: number): Shipment[] => {
	  ...
	  for (let i = 0; i < count; i++) {
		  ...
		  const estimatedDeliveryDate = randomDate(new Date(), new Date(Date.now() + 30 * 24 * 60 * 60 * 1000));
		  ...
		  shipments.push({
			  ...
			  ...(status === "DELIVERED" && { actualDeliveryDate: randomDate(new Date(estimatedDeliveryDate), new Date(new Date(estimatedDeliveryDate).getTime() + 2 * 24 * 60 * 60 * 1000)) }),
			  currentLocation: {
  ...
  const generateUsers = (count: number): User[] => {
	  const users: User[] = [];
	  const roles = [UserRole.ADMIN, UserRole.CUSTOMER, UserRole.MANAGER, UserRole.SUPPLIER];
	  ...
	  for (let i = 0; i < count; i++) {
		  ...
		  users.push({
			  ...
			  kycStatus: randomElement([KYCStatus.APPROVED, KYCStatus.IN_PROGRESS, KYCStatus.NOT_STARTED, KYCStatus.PENDING_REVIEW, KYCStatus.REJECTED]),
			  onboardingStatus: randomElement([OnboardingStatus.COMPLETED, OnboardingStatus.IN_PROGRESS, OnboardingStatus.NOT_STARTED, OnboardingStatus.PENDING]),
			  ...
			  integrations: {
				  ecommerce: {
					  ...
					  storeUrl: randomBoolean() ? `${firstName.toLowerCase()}-store.myshopify.com` : null
				  },
				  erp_crm: {
					  enabled: randomBoolean(),
  ...
  const generateShopifyOrders = (count: number): ShopifyOrder[] => {
	  ...
	  const fulfillmentStatuses = ["fulfilled", "partial", "unfulfilled", "pending", null];
	  const financialStatuses = ["paid", "pending", "authorized", "partially_paid", "refunded", "voided"];
	  ...
	  for (let i = 0; i < count; i++) {
		  ...
		  for (let j = 0; j < itemCount; j++) {
			  items.push({
				  ...
				  lotNumber: randomBoolean() ? `LN${randomNumber(1000, 9999)}` : undefined,
				  serialNumber: randomBoolean() ? `SN${randomNumber(10000, 99999)}` : undefined
			  });
		  }
		  
		  const scheduledDate = randomDate(new Date(2023, 0, 1), new Date(2024, 0, 1));
		  
		  movements.push({
			  id: `movement-${i + 1}`,
			  type: type,
			  sourceLocationId: type === "RECEIVING" ? `supplier-${randomNumber(1, 10)}` : `warehouse-${randomNumber(1, 5)}`,
			  destinationLocationId: type === "DISPATCH" ? `customer-${randomNumber(1, 20)}` : `warehouse-${randomNumber(1, 5)}`,
			  items: items,
			  status: status,
			  scheduledDate: scheduledDate,
			  ...(status === "COMPLETED" && { completedDate: randomDate(new Date(scheduledDate), new Date()) }),
	  ...
	  for (let i = 0; i < count; i++) {
		  ...
		  if (status === "IN_PROGRESS" || status === "COMPLETED") {
			  ...
			  for (let j = 0; j < checkCount; j++) {
				  const checkTypes = ["Visual Inspection", "Performance Test", "Dimensional Measurement", "Functional Test", "Stress Test"];
				  ...
				  for (let k = 0; k < paramCount; k++) {
					  ...
					  parameters.push({
						  name: `Parameter ${k + 1}`,
						  expected: type === "STRING" ? randomElement(["Red", "Blue", "Green", "Yellow", "Black", "White"]) : randomNumber(10, 100),
						  actual: type === "STRING" ? randomElement(["Red", "Blue", "Green", "Yellow", "Black", "White"]) : randomNumber(10, 100),
				  ...
				  qualityChecks.push({
					  ...
					  notes: randomBoolean() ? "All parameters within acceptable range." : undefined
				  });
			  }
		  }
		  ...
		  orders.push({
			  ...
			  endDate: endDate,
			  ...(status !== "PLANNED" && {
				  actualStartDate: randomDate(new Date(startDate), new Date(new Date(startDate).getTime() + 2 * 24 * 60 * 60 * 1000)),
				  ...(status === "COMPLETED" && { actualEndDate: randomDate(new Date(endDate), new Date(new Date(endDate).getTime() + 2 * 24 * 60 * 60 * 1000)) })
			  }),
  ...
  const generateNotifications = (count: number): Notification[] => {
	  ...
	  for (let i = 0; i < count; i++) {
		  ...
		  switch (type) {
			  case "GENERAL":
				  title = randomElement(["Welcome!", "New Feature Available", "System Maintenance", "Important Announcement"]);
			  ...
			  case "INVENTORY_ALERT":
				  const productName = randomElement(productNamesByCategory[randomElement(productCategories)]);
			  ...
			  case "INTEGRATION_SYNC":
				  ...
				  data = { integration: randomElement(["Shopify", "SAP", "IoT", "Power BI"]) };
				  break;
	  ...
	  get: {
		  ...
		  "/freights": (): Freight[] => [
			  ...
			  },
			  {
				  ...
				  volume: 40,
				  hazardous: false,
				  specialInstructions: "Heavy machinery; use appropriate lifting equipment.",
			  },
		  ],
		  "/transports": (): Transport[] => [
			  ...
			  },
			  {
				  ...
				  type: "TRUCK",
				  capacity: 1500,
				  currentLocation: { latitude: 33.4484, longitude: -112.0740 },
				  status: "AVAILABLE",
				  carrierId: "carrier-4",
			  },
			  {
		  ...
		  "/users": (params: any) => {
			  ...
			  const users = startIndex < totalUsers
		  ...
		  "/shopify/shop": (): ShopifyShop => mockShops[0],
