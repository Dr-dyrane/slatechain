...
import { InventoryItem, InventoryState, ManufacturingOrder, StockMovement, Warehouse } from "@/lib/types";
...
export const fetchWarehouses = createAsyncThunk("inventory/fetchWarehouses", async (_, thunkAPI) => {
	try {
	  const response = await apiClient.get<Warehouse[]>("/warehouses")
	  return response
	} catch (error: any) {
	  return thunkAPI.rejectWithValue(error.message || "Failed to fetch warehouses")
	}
  })
  
  export const addWarehouse = createAsyncThunk(
	"inventory/addWarehouse",
	async (warehouse: Omit<Warehouse, "id" | "createdAt" | "updatedAt">, thunkAPI) => {
	  try {
		const response = await apiClient.post<Warehouse>("/warehouses", warehouse)
		return response
	  } catch (error: any) {
		return thunkAPI.rejectWithValue(error.message || "Failed to add warehouse")
	  }
	},
  )
  
  export const updateWarehouse = createAsyncThunk("inventory/updateWarehouse", async (warehouse: Warehouse, thunkAPI) => {
	try {
	  const response = await apiClient.put<Warehouse>(`/warehouses/${warehouse.id}`, warehouse)
	  return response
	} catch (error: any) {
	  return thunkAPI.rejectWithValue(error.message || "Failed to update warehouse")
	}
  })
  
  export const deleteWarehouse = createAsyncThunk("inventory/deleteWarehouse", async (id: string, thunkAPI) => {
	try {
	  await apiClient.delete(`/warehouses/${id}`)
	  return id
	} catch (error: any) {
	  return thunkAPI.rejectWithValue(error.message || "Failed to delete warehouse")
	}
  })
  
  // Stock Movement Thunks
  export const fetchStockMovements = createAsyncThunk("inventory/fetchStockMovements", async (_, thunkAPI) => {
	try {
	  const response = await apiClient.get<StockMovement[]>("/stock-movements")
	  return response
	} catch (error: any) {
	  return thunkAPI.rejectWithValue(error.message || "Failed to fetch stock movements")
	}
  })
  ...
  export const createStockMovement = createAsyncThunk(
	...
	async (movement: Omit<StockMovement, "id">, thunkAPI) => {
	  try {
		const response = await apiClient.post<StockMovement>("/stock-movements", movement)
		return response
	  } catch (error: any) {
		return thunkAPI.rejectWithValue(error.message || "Failed to create stock movement")
	  }
	},
  )
  
  // Manufacturing Order Thunks
  export const fetchManufacturingOrders = createAsyncThunk("inventory/fetchManufacturingOrders", async (_, thunkAPI) => {
	try {
	  const response = await apiClient.get<ManufacturingOrder[]>("/manufacturing-orders")
	  return response
	} catch (error: any) {
	  return thunkAPI.rejectWithValue(error.message || "Failed to fetch manufacturing orders")
	}
  })
  ...
  export const createManufacturingOrder = createAsyncThunk(
	...
	async (order: Omit<ManufacturingOrder, "id">, thunkAPI) => {
	  try {
		const response = await apiClient.post<ManufacturingOrder>("/manufacturing-orders", order)
		return response
	  } catch (error: any) {
		return thunkAPI.rejectWithValue(error.message || "Failed to create manufacturing order")
	  }
	},
  )
  ...
  export const updateManufacturingOrder = createAsyncThunk(
	...
	async (order: ManufacturingOrder, thunkAPI) => {
	  try {
		const response = await apiClient.put<ManufacturingOrder>(`/manufacturing-orders/${order.id}`, order)
		return response
	  } catch (error: any) {
		return thunkAPI.rejectWithValue(error.message || "Failed to update manufacturing order")
	  }
	},
  )
  ...
  export const deleteManufacturingOrder = createAsyncThunk(
	...
	async (id: string, thunkAPI) => {
	  try {
		...
		return thunkAPI.rejectWithValue(error.message || "Failed to delete manufacturing order")
	  }
	},
  )
		...
		builder
		  ...
		  builder
			...
			.addCase(updateManufacturingOrder.fulfilled, (state, action) => {
			  const index = state.manufacturingOrders.findIndex((o) => o.id === action.payload.id)
			  if (index !== -1) {
				state.manufacturingOrders[index] = action.payload
			  }
			})
			.addCase(deleteManufacturingOrder.fulfilled, (state, action) => {
			  state.manufacturingOrders = state.manufacturingOrders.filter((o) => o.id !== action.payload)
			})
	},
});
