...
export async function PUT(req: Request, { params }: { params: { stepId: string } }) {
  const { headers, limited } = await withRateLimit(req, "onboarding_update", 10)
  ...
  try {
    ...
    // Update step
    const stepIndex = onboarding.steps.findIndex((step : any) => step.stepId === stepId)
    if (stepIndex === -1) {
    ...
    if (status === OnboardingStepStatus.COMPLETED && onboarding.currentStep === stepId) {
