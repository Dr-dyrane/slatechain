...
export async function PUT(req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (!admin || (admin.role !== UserRole.ADMIN && decoded.userId !== params.id)) {
      return NextResponse.json({ code: "FORBIDDEN", message: "Unauthorized access" }, { status: 403, headers })
    }
    ...
    if (!user) {
      return NextResponse.json({ code: "NOT_FOUND", message: "User not found" }, { status: 404, headers })
    }
    ...
    if (email && email !== user.email) {
      ...
      if (existingUser) {
        return NextResponse.json({ code: "EMAIL_EXISTS", message: "Email already in use" }, { status: 400, headers })
      }
      user.email = email
    ...
    return NextResponse.json({ code: "SERVER_ERROR", message: "Failed to update user" }, { status: 500, headers })
  }
}
...
export async function DELETE(req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (decoded.userId === params.id) {
      return NextResponse.json(
        { code: "INVALID_OPERATION", message: "Cannot delete your own account" },
        { status: 400, headers },
      )
    }
    ...
    return NextResponse.json({ success: true, message: "User deleted successfully" }, { headers })
  } catch (error) {
    console.error("Delete User Error:", error)
    return NextResponse.json({ code: "SERVER_ERROR", message: "Failed to delete user" }, { status: 500, headers })
  }
}
